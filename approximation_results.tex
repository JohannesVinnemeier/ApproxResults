\begin{center}
    \Large \textbf{Approximation Results for Smooth Maps} \\[.5em]
    \large\textsc{Johannes Vinnemeier} \\
\end{center}

\vspace{1em}

{\small \noindent \textsc{Abstract.} This article is supposed to serve as an accessible, self-contained reference for various approximation results commonly used in differential topology. In particular, we give elementary proofs of approximation results for immersions, injective immersions, embeddings and transversal maps. All results are formulated in terms of the Whitney $C^\infty$-topology and are stated in a fairly general form, without assuming additional topological conditions such as compactness or empty boundary of the manifolds involved. Furthermore, we provide a general result describing how these approximation theorems can be translated into existence theorems for homotopies, which is often how they are used in practice.} 

\vspace{1em}

% Let $M$ and $N$ be smooth manifolds and consider the set $C^\infty(M,N)$ of smooth maps from $M$ to $N$. On this set one can define the so called Whitney topologies, which provide a precise language to make sense of statements such as "slightly perturbing $f$ yields a map transversal to a given submanifold". In the following we will proof a number of useful approximation results of this kind concerning immersions, one-to-one immersions, embeddings and transversal maps. Moreover we provide a general result on how one can translate these theorems into existence theorems for homotopies. 

\subsection{Formulation of the Approximation Theorems}

Unless otherwise stated all manifolds (and submanifolds) are allowed to have empty or non-empty boundary. Let us begin by recalling the definition of the Whitney topologies. First we define a topology on the space $C^k(M,N)$ of $C^k$-maps from $M$ to $N$. We will be using the definition from \cite{hirsch2012differential} based on local charts. Suppose we are given the following data.
...
\noindent Then if $f \in C^k(M,N)$ with $f(K_\alpha) \subseteq V_\alpha$ for each $\alpha \in J$, we define a basic $C^k$-neighbourhood
\[
...
\]
The sum is taken over multiindices $\beta \in \N_0^n$, where $n = \mathrm{dim}(M)$. The collection of all such basic open neighbourhoods form the basis for a topology on $C^k(M,N)$ called the Whitney $C^k$-topology (it is easy to see that the intersection of two such neighbourhoods is again of the same form). The  Whitney $C^\infty$-topology on $C^\infty(M,N)$ is then simply defined as the topology induced by all inclusion maps $C^\infty(M,N) \to C^k(M,N)$. So explicitly a basis for the $C^\infty$-topology is given by subsets of the form $\mathcal{N}^k(f,\Phi,\Psi,K,\epsilon) \cap C^\infty(M,N)$.

A first interesting approximation result is the following.

\begin{theorem}
    \label{whitney_top:thm:C0_is_dense}
    The set $C^k(M,N)$ is dense in $C^0(M,N)$, $0\leq k \leq \infty$.
\end{theorem}
...
\begin{theorem}[Immersions]
    ...
    Let $M^n$ and $N^p$ be manifolds, $p \geq 2n$. Then the set of immersions is dense in $C^\infty(M,N)$. Moreover if $f \colon M \to N$ is already an immersion on some closed subset $A \subseteq M$ there exists an immersion $g \colon M \to N$ with $g|_A = f|_A$ arbitrarily close to $f$.
\end{theorem}
...
\begin{theorem}[One-to-one Immersions]
    ...
    Let $M^n$ and $N^p$ be manifolds, $p \geq 2n + 1$. Then the set of one-to-one immersions is dense in $C^\infty(M,N)$. Moreover if $f \colon M \to N$ is already a one-to-one immersion on some neighbourhood $U \subseteq M$ of a closed subset $A \subseteq M$ there exists a one-to-one immersion $g \colon M \to N$ with $g|_A = f|_A$ arbitrarily close to $f$.
\end{theorem}
...
\begin{corollary}[Embeddings]
    ...
    Let $M^n$ and $N^p$ be manifolds, $p \geq 2n+1$. Then the set of proper embeddings is dense in $\mathrm{Prop}^\infty(M,N)$. Moreover if $f \colon M \to N$ is a proper map which is a one-to-one immersion on some neighbourhood $U \subseteq M$ of a closed subset $A \subseteq M$ there exists a proper embedding $g \colon M \to N$ with $g|_A = f|_A$ arbitrarily close to $f$.
\end{corollary}
...
\begin{lemma}
    ...
    Let $f \colon M \to N$ be a smooth map, which is a one-to-one immersion on a compact set $A \subseteq M$. Then $f$ is already a one-to-one immersion on some neighbourhood $U \subseteq M$ of $A$. 
\end{lemma}

\begin{proof}
    It is clear that $f$ is an immersion in a neighbourhood of $A$. Suppose that $f$ is not one-to-one in any neighbourhood of $A$. Then we can use compactness of $A$ to construct sequences $(a_n)_{n \in \N}$ and $(a_n^\prime)_{n \in \N}$ in $M$ with $a_n \neq a_n^\prime$ and $f(a_n) = f(a_n^\prime)$, which converge to points $z$ and $z^\prime$ in $A$ respectively. This implies $f(z) = f(z^\prime)$ and since $f$ is one-to-one on $A$ we conclude $z = z^\prime$. However, since $f$ is an immersion at $z$ it must be one-to-one on a neighbourhood of $z$ contradicting $a_n \neq a_n^\prime$.
\end{proof}
...
\begin{proposition}
    ...
    Let $\mathcal{U} \subseteq  C^\infty(M,N)$ be some open subset and $f \in \mathcal{U}$. Then for any $g \in \mathcal{U}$ sufficiently close to $f$ there exists a homotopy $h_t \colon M \to N$, $0 \leq t \leq 1$, from $f$ to $g$ with $h_t \in \mathcal{U}$ for all $0 \leq t \leq 1$. Moreover if $f|_A = g|_A$ for some closed subset $A \subseteq M$ we can choose this homotopy rel $A$.
\end{proposition}

Together all of these results comprise a very efficient machinery for approaching problems in differential topology. As an example we proof the following.

\begin{corollary}
    Let $M^n$ and $N^p$ be compact manifolds, $p \geq 2n+3$. Then two embeddings $f,g \colon M \to N$ are isotopic if and only if they are homotopic.
\end{corollary}
...
\subsection{Proofs of the Approximation Theorems}
...
\begin{proof}[Proof of Theorem \ref{whitney_top:thm:approx_immersions}]
    ...
    \end{enumerate}
    The existence of the data described above follows essentially from paracompactness of $M$. We will construct $g$ as the limit of an inductively defined sequence $f_n \colon M \to N$, $n \geq 0$, with $f_0 := f$, satisfying the following properties.
...
\begin{proof}[Proof of Theorem \ref{whitney_top:thm:approx_one_to_one_immersions}]
    ...
    \end{enumerate}
    Again the existence of this data follows from paracompactness of $M$. For (\ref{whitney_top:eq:one_to_one_immersion_cover_property5}) recall that immersions are locally embeddings. We now inductively inductively define a sequence $f_n \colon M \to N$, $n \geq 0$, with $f_0 := f$, satisfying the following properties.
    \begin{enumerate}[1)]
...
Next we proof Theorem \ref{whitney_top:thm:approx_transversality} about transversality. As for the proof of Theorem \ref{whitney_top:thm:approx_immersions} we start with a local result, which is stated in Lemma \ref{whitney_top:lem:approx_transversality_lemma} below. In the proof of Lemma \ref{whitney_top:lem:approx_transversality_lemma} we make use of the following result.

    \begin{lemma}
        \label{on_mnf_with_boundary:lem:preimage_of_submersion}
            Let $M,N$ be manifolds where $M$ has no boundary and let $f \colon M \to N$ be a submersion. Given a submanifold $S \subseteq N$ the preimage $f^{-1}(S) \subseteq M$ is a submanifold with boundary 
            \[
            \partial f^{-1}(S) = f^{-1}(\partial S)
            \]
            and $\mathrm{codim}(f^{-1}(S)) = \mathrm{codim}(S)$. 
        \end{lemma}
        \begin{proof}
            First we may also assume $\partial N = \emptyset$, otherwise embed $N$ into its double. Let $p \in f^{-1}(\partial S), q = f(p) \in \partial S$ and choose a chart $\psi \colon V \to N$ around $q$, such that 
            \[
            \psi^{-1}(S) = V \cap \{(x_1,\ldots,x_n) \mid x_1 \leq 0, x_2 = \ldots = x_{s+1} = 0\},
            \]
            where $n = \mathrm{dim}(N)$ and $s = \mathrm{codim}(S)$. Since $f$ is a submersion we can find a chart $\phi \colon U \to M$ around $p$, such that 
            \[
            \psi^{-1} \circ f \circ \phi \colon U \to V
            \]
            is given by projection onto the first $n$ coordinates, i.e. $(x_1,\ldots,x_m) \mapsto (x_1,\ldots, x_n)$ for $m = \mathrm{dim}(M) \geq n$. Thus in the coordinates of $\phi$, $f^{-1}(S)$ is given by 
            \[
            \phi^{-1}(f^{-1}(S)) = U \cap \{(x_1,\ldots,x_m) \mid x_1 \leq 0, x_2 = \ldots = x_{s+1} = 0\},
            \]
            such that $\phi$ is a submanifold chart for $f^{-1}(S)$ around $p$ and $\partial(f^{-1}(S) \cap \phi(U)) = f^{-1}(S) \cap \phi(U)$. Similiarly we construct submanifold charts around points $p \in f^{-1}(\mathrm{int}(S))$ from which the statement follows.
    \end{proof}

    The above Lemma becomes false if we also allow $M$ to have boundary. Consider for example the $2$-dimensional half space $\H^2 := \{(x,y) \in \R^2 \mid x \leq 0\}$ and the submersion $f \colon \H^2 \to \R, \; (x,y) \mapsto y$. Then $f^{-1}([0,1])$ is not a submanifold.

\begin{lemma}
    \label{whitney_top:lem:approx_transversality_lemma}
    Let $M$ be a manifold, $f \colon M \to \H^p$ a smooth function, $S \subseteq \H^p$ a submanifold and $\lambda \colon M \to (0,\infty)$ a positive function. Then for almost all vectors $b \in \H^p$ the map
...
\begin{proof}
    ...
    \]
    is clearly a submersion. We now proof a version of the paramteric transversality theorem which also holds in the case where $M$ and $S$ have boundary. Using a partition of unity we can extend $F$ to some map $F^\prime \colon M^\prime \times B \to \R^p$, where $M^\prime$ is some boundaryless manifold containing $M$ as a codimension $0$ submanifold. Since $F^\prime$ is a submersion on $M \times B$ we can find some open neighbourhood $U \subseteq M^\prime \times B$ of $M \times B$ on which $F^\prime$ is a submersion. Let 
    \[
    G := F^\prime|_U \colon U \to \R^p
    \]
    denote this submersion. $U$ is a boundaryless manifold so by Lemma \ref{on_mnf_with_boundary:lem:preimage_of_submersion} the preimage $W:= G^{-1}(S) \subseteq U$ is a submanifold. Now consider the map 
    \[
